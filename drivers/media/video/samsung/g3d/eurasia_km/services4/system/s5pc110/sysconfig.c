...
{
	PVR_DPF((PVR_DBG_WARNING,"EnableSGXClocks: 3D clk enabled\n"));
    clk_enable(g3d_clock);
    //printk("\n=== 3D clk enabled dummy\n");
    return PVRSRV_OK;
}

PVRSRV_ERROR DisableSGXClocks(void)
{
    PVR_DPF((PVR_DBG_WARNING,"DisableSGXClocks: 3D clk disabled\n"));	
...
PVRSRV_ERROR SysInitialise(IMG_VOID)
{
	...
	if (IS_ERR(g3d_clock)) 
	{
		...
		PVR_DPF((PVR_DBG_ERROR,"SysInitialise: 3D failed to find g3d clock source-enable"));
									...
									PVRSRV_DEV_POWER_STATE		eCurrentPowerState)
{
	...
	{
		PVR_DPF((PVR_DBG_WARNING, "SysDevicePrePowerState: SGX Entering state D3"));
									 ...
									 PVRSRV_DEV_POWER_STATE	eCurrentPowerState)
{
	...
	{
		PVR_DPF((PVR_DBG_WARNING, "SysDevicePostPowerState: SGX Leaving state D3"));
		eError = EnableSGXClocks();
		PVRSRVSetDCState(DC_STATE_FLUSH_COMMANDS);
		PVRSRVSetDCState(DC_STATE_NO_FLUSH_COMMANDS);
	}

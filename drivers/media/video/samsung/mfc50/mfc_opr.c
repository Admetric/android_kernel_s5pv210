...
#define       ENABLE_RESET_CONTEXT_BUFFER                                  1
...
static void mfc_set_encode_init_param(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	{
	/* Set "MFC_RC_FRAME_RATE" value according to MFC F/W Type (Android vs Main) */
#if     ENABLE_FW_MAIN
		enc_init_mpeg4_arg->in_RC_framerate = enc_init_mpeg4_arg->in_RC_framerate * 1000;
		mfc_info("0-ENC Framerate = (%d : 0x%08x) \n", enc_init_mpeg4_arg->in_RC_framerate, READL(MFC_RC_FRAME_RATE));
		WRITEL(enc_init_mpeg4_arg->in_RC_framerate, MFC_RC_FRAME_RATE);
		mfc_info("1-ENC Framerate = (%d : 0x%08x) \n", enc_init_mpeg4_arg->in_RC_framerate, READL(MFC_RC_FRAME_RATE));
#else
		WRITEL(enc_init_mpeg4_arg->in_RC_framerate, MFC_RC_FRAME_RATE);
#endif
...
MFC_FW_RESET:
	...
	if (mfc_cmd_host2risc(H2R_CMD_SYS_INIT, fw_buf_size, 0, 0, 0) == false)
	{
		...
		return MFCINST_ERR_FW_LOAD_TIMEOUT_FAIL;
	}

	nIntrRet = mfc_wait_for_done(R2H_CMD_SYS_INIT_RET);
	if (nIntrRet != R2H_CMD_SYS_INIT_RET)
	{
		mfc_err("R2H_CMD_SYS_INIT_RET FAIL............IntrRet(%d)\n", nIntrRet);
...
static int mfc_get_inst_no(mfc_inst_ctx *mfc_ctx, unsigned int context_addr, int context_size)
{
					...
					context_size) == false)
	{
		...
		return MFCINST_ERR_OPEN_TIMEOUT_FAIL;
	}
	...
	if (mfc_wait_ret != R2H_CMD_OPEN_INSTANCE_RET)	{
		...
		if(mfc_wait_ret == R2H_CMD_TIMEOUT)
			return MFCINST_ERR_OPEN_TIMEOUT_FAIL;
		else
...
MFC_ERROR_CODE mfc_init_encode(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	if (mfc_ctx->InstNo == -1)
	{
		...
		return MFCINST_ERR_INST_NUM_EXCEEDED;
	}
	else if(mfc_ctx->InstNo < 0)
	{
		mfc_err("MFCINST_ERR_GET_INST_FAIL\n");
		return MFCINST_ERR_GET_INST_FAIL;		
	}
...
static MFC_ERROR_CODE mfc_encode_header(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	/* Check MFC status */
	bMfc_empty = is_mfc_empty_status();
	...
	if (nIntrRet == R2H_CMD_TIMEOUT)
	{
		mfc_err("MFCINST_ERR_ENC_EXE_TIME_OUT.....Intr Code (%d)\n", nIntrRet);
		...
		mfc_err("MFC_RC_FRAME_RATE = %d \n", READL(MFC_RC_FRAME_RATE));
		return MFCINST_ERR_ENC_HEADER_DECODE_FAIL;
	}
	else if(nIntrRet != R2H_CMD_SEQ_DONE_RET){
		mfc_warn("MFCINST_WARN_ENC_SEQ_HEADER.........Intr Code(%d)\n", nIntrRet);
	}	
...
static MFC_ERROR_CODE mfc_encode_one_frame(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	nReturnErrCode = mfc_return_code();
	if (interrupt_flag == R2H_CMD_TIMEOUT){
...
MFC_ERROR_CODE mfc_set_sleep()
{
	...
	{
		mfc_err("R2H_CMD_SLEEP TIMEOUT FAIL\n");
		return MFCINST_SLEEP_TIMEOUT_FAIL;
	}

	ret = mfc_wait_for_done(R2H_CMD_SLEEP_RET);
	if (ret != R2H_CMD_SLEEP_RET)
	{
		mfc_err("R2H_CMD_SLEEP_RET FAIL........IntrRet(%d)\n", ret);
		if(ret == R2H_CMD_TIMEOUT)
			return MFCINST_SLEEP_TIMEOUT_FAIL;
		else
...
MFC_ERROR_CODE mfc_set_wakeup()
{
	...
	if (mfc_cmd_host2risc(H2R_CMD_WAKEUP, 0, 0, 0, 0) == false)
	{
		...
		return MFCINST_WAKEUP_TIMEOUT_FAIL;
	}
	...
	{
		mfc_err("R2H_CMD_WAKEUP_RET FAIL........IntrRet(%d)\n",ret);
		if(ret == R2H_CMD_TIMEOUT)
			return MFCINST_WAKEUP_TIMEOUT_FAIL;
		else
...
#if     ENABLE_RESET_CONTEXT_BUFFER
...
BOOL	is_mfc_empty_status(void)
{
	bool	is_mfc_empty	= FALSE;
	unsigned long timeo = jiffies;
	unsigned int mc_status;
	...
	/* Check MFC's Status */
	/* wait until MFC status becomes "EMPTY" */
	while (time_before(jiffies, timeo))
	{
		mc_status = READL(MFC_MC_STATUS);
		if (mc_status == 0)
        ...
        mfc_err("Can't execute MFC command because MFC is busy....!! \n");

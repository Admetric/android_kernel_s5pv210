...
//  sspi->cur_mode |= (spi->mode & SPI_READY);
	   S3C_SETGPIOPULL(sspi);
 	   printk ("[%s : %d]SPI-%d  GPBCON: 0x%x  GPBDAT: 0x%x  GPBPUD: 0x%x  GPBDRV: 0x%x \n",__func__, __LINE__, \
			sspi->spi_mstinfo->pdev->id,readl(S5PV210_GPBCON), readl(S5PV210_GPBDAT), readl(S5PV210_GPBPUD), readl(S5PV210_GPBDRV));
	}
#endif
